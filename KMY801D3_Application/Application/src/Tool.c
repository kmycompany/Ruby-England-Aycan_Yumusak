...
void GetCurrentDateTime(char* pDate, char* pTime, char* pDelimiter)
{
	...
	date=tm_ptr->tm_mday;
	hour=tm_ptr->tm_hour;
	minite=tm_ptr->tm_min;
	second=tm_ptr->tm_sec;
	*/
	if(NULL == pDelimiter)
	{
		sprintf(pDate, "%04d%02d%02d", year, month, date);
		sprintf(pTime, "%02d%02d%02d", hour, minite, second);
	}
	else
	{
		sprintf(pDate, "%04d-%02d-%02d", year, month, date);
...
{
	memcpy(pDate1, pDate, 4);
	memcpy(pDate1+4, pDate+5, 2);
	memcpy(pDate1+6, pDate+8, 2);

	memcpy(pTime1, pTime, 2);
	memcpy(pTime1+2, pTime+3, 2);
	memcpy(pTime1+4, pTime+6, 2);
}
...
int ASCtoBCD(const unsigned char* pAsc, unsigned char* pBcd)
{
	...
	while( i < nLen )
	{
		if( (pAsc[i] >= 0x30 && pAsc[i] <= 0x39))
			H = ( pAsc[i] - 0x30 ) * 16 ;
		else
			H = ( pAsc[i] - 0x37 ) * 16 ; //0x37 = 'A' - A
		
		if(pAsc[i+1] >= 0x30 && pAsc[i+1] <= 0x39)
			L = (pAsc[i+1]-0x30);
		else
			L = (pAsc[i+1]-0x37);
		pBcd[p++] = H + L ;
...
void BCDtoASC(const unsigned char* pBcd, int nBcdLen, unsigned char* pAsc )
{
	...
	for (i=0 ; i < nBcdLen * 2 ; i++)
	{
		...
		pAsc[i] = ch + ( ( ch > 9 ) ? 'A'- 0x0A : '0' ) ;
	}
}

void ASCtoHex(const char* pAsc, char* pHex)
{
	long lMoney = atoi(pAsc) ;

	pHex[3] =  lMoney%256 ;
	pHex[2] =  lMoney/256 ;
	pHex[1] =  lMoney/(256*256) ;
	pHex[0] =  lMoney/(256*256*256) ;
}
