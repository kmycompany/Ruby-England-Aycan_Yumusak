...
static char display_papa(u8 *buff,u8 *Title,u8 *buffEND)
{
	...
	while(1){
		...
		switch(keyval){
			case KEY_UP:
			if(curR){				//current row is not the first row
				curR--;
				if(curHR>STARTROW){					//if is high light row is not the first row, only want to switch the Highlight row
					oldHR=curHR;
					curHR--;
			...
			}else{				//current row is the first row, the next row is the last row
			...
			case KEY_DOWN:
			if(curR<TotalR-1){				//current row is not the last row
				curR++;
				if(curHR<ENDROW&&curHR<TotalR){					//if is high light row is not the first row, only want to switch the Highlight row
					oldHR=curHR;
					curHR++;
					DispHighLightRow(oldHR);
					DispHighLightRow(curHR);
			...
			}else{				//current row is the last row, next row is the first row
