...
void MenuHandler_Language2_Pic(struct MenuOperate_Language2_Pic *MenuOp)
{
	...
	unsigned char dispbuffOther[MaxByteOneLine+2];
	unsigned char keyval;
	unsigned char dispbuff[50];
	unsigned long MilliSeconds;
	unsigned char j=0;

	MilliSeconds=kmy_TimeGetMilliseconds();
	...
	while(1){
		...
		// ----for long item crolling display ----
			if(Language[0]=='1')// English
			{
				if(strlen((char*)((*MenuOp).Menu)->MenuStrOther )>17)
				{
					if(kmy_TimeGetMilliseconds()>MilliSeconds+1000)// 1s 
					{
						MilliSeconds=kmy_TimeGetMilliseconds();
						memset(dispbuff,0,sizeof(dispbuff));
						strcpy((char*)dispbuff,(char*)((*MenuOp).Menu)->MenuStrOther);
	
						clear_area_to_row((*MenuOp).curFlashRow,(*MenuOp).curFlashRow);
						text_out_to_row((*MenuOp).curFlashRow,dispbuff+j);
						DispHighLightRow ( (*MenuOp).curFlashRow);
						DisplayMenuPercent ( (struct MenuOperate *) MenuOp);
						j++;
						if(j>strlen((char*)((*MenuOp).Menu)->MenuStrOther )-20)j=0;
					}
				}
			}
			else // local language
			{
				if(strlen((char*)((*MenuOp).Menu)->MenuStrLocal )>17)
				{
					if(kmy_TimeGetMilliseconds()>MilliSeconds+1000)// 1s 
					{
						MilliSeconds=kmy_TimeGetMilliseconds();
						memset(dispbuff,0,sizeof(dispbuff));
						strcpy((char*)dispbuff,(char*)((*MenuOp).Menu)->MenuStrLocal);
						...
						DisplayMenuPercent ( (struct MenuOperate *) MenuOp);
						j++;
						if(j>strlen((char*)((*MenuOp).Menu)->MenuStrLocal )-16)j=0;
					}
				}
			}
...
void MenuHandler_Orders (struct MenuOperate_Orders *MenuOp)
{
	unsigned char i;
	unsigned char dispbuffLocal[MaxByteOneLine + 2];
	unsigned char dispbuffOther[MaxByteOneLine + 2];
		...
		switch (keyval)
		{
			...
			case KEY_UP:
				if ( (*MenuOp).Menucnt)  				//未到第一个菜单
				{
					...
					{
						(*MenuOp).curFlashRow--;
						(*MenuOp).flashflag = FlushOnlyHightRowFlag | FlushUpkeyFlag;
					}
				}
				else 				//到了第一个菜单，下一个菜单是最后一个。
				{
			...
			case KEY_DOWN:
				...
				if ( (*MenuOp).Menucnt < (*MenuOp).MenuTotal - 1) //未到最后一个菜单
				{
					...
					if ( (*MenuOp).curFlashRow < (*MenuOp).EndLine - 1 && (*MenuOp).curFlashRow < (*MenuOp).MenuTotal + (*MenuOp).StartLine - 1)
					{
						(*MenuOp).curFlashRow++;
						(*MenuOp).flashflag = FlushOnlyHightRowFlag | FlushDownkeyFlag;
					}
				}
				else									//到了最后一个菜单，下一个菜单是第一个。
				{
