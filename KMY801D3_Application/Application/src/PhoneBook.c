...
static s8 PhoneHandleNameXiuGai(s8 *msg)
{
	...
	static struct FunctionMenuAddIco Menu[]={
	...
	struct OperateAddIco MenuOp={Menu,sizeof(Menu) / sizeof(Menu[0]),0,0,0,MaxRowOneScr,1,0};
...
static char HandlePhoneSelection(char numPhone)
{
	...
	char dispbuff[11];
	u8 retval;
	static struct FunctionMenuAddIco2 Menu[]={
	...
	struct OperateAddIco2 MenuOp={Menu,sizeof(Menu) / sizeof(Menu[0]),0,0,0,MaxRowOneScr, 1,0,0};
...
static char PhoneAdd(char *msg)
{
	...
	struct OperateAddIco MenuOp={Menu,sizeof(Menu) / sizeof(Menu[0]),0, 0,0,MaxRowOneScr, 1,0,0};
