...
unsigned char FormInput(struct sFormInput *pFI,unsigned char TitleNumber)
{
	...
	pt=pFI;
	for(cnt=i=0;i<TitleNumber-2;i++){	//得到最长Title 的bytes 大小
	...
	while(1){
		...
		switch(keyval){
			...
			case KEY9:
			case KEYXING:
			if(strlen((const char*)(pt->TitleResult))>=pt->MaxInputSize){KeyIsResponse();continue;}

			if(inputMode==INPUT_MODE_123){
				temp=strlen((const char*)(pt->TitleResult));
				if(temp>=pt->MaxInputSize){KeyIsResponse();continue;}
