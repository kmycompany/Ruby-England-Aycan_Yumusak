...
void My_clear_area_to_row (unsigned int startrow, unsigned int endrow)
{
	kmy_LcdDrawRect (0, My_ALineContainDotLine * startrow, 132, My_ALineContainDotLine * (endrow + 1), mode_White);
}
...
{
	//draw_line(0,ALineContainDotLine*(row+1)-1,132,ALineContainDotLine*(row+1)-1,1);
}
...
{
	//printf("xstart=%d,ystart=%d,xlen=%d,ylen=%d\r\n",xstart, ystart, xlen, ylen);
	kmy_LcdDrawRect(xstart,ystart,xstart+xlen-1,ystart+ylen-1,mode);
}
